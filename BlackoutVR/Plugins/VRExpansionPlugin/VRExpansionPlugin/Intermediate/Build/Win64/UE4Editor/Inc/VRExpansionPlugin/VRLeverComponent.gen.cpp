// Copyright 1998-2017 Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "GeneratedCppIncludes.h"
#include "Public/Interactibles/VRLeverComponent.h"
PRAGMA_DISABLE_OPTIMIZATION
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable : 4883)
#endif
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeVRLeverComponent() {}
// Cross Module References
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UDelegateFunction_VRExpansionPlugin_VRLeverStateChangedSignature__DelegateSignature();
	UPackage* Z_Construct_UPackage__Script_VRExpansionPlugin();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_AdvancedPhysicsSettings();
	VREXPANSIONPLUGIN_API UClass* Z_Construct_UClass_UVRLeverComponent();
	VREXPANSIONPLUGIN_API UScriptStruct* Z_Construct_UScriptStruct_FBPAdvGripPhysicsSettings();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_ClosestPrimarySlotInRange();
	VREXPANSIONPLUGIN_API UClass* Z_Construct_UClass_UGripMotionControllerComponent_NoRegister();
	COREUOBJECT_API UScriptStruct* Z_Construct_UScriptStruct_FTransform();
	COREUOBJECT_API UScriptStruct* Z_Construct_UScriptStruct_FVector();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_ClosestSecondarySlotInRange();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_DenyGripping();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_FreeGripType();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EGripCollisionType();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_GetInteractionSettings();
	VREXPANSIONPLUGIN_API UScriptStruct* Z_Construct_UScriptStruct_FBPInteractionSettings();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_GripBreakDistance();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_GripDamping();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_GripLateUpdateSetting();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EGripLateUpdateSettings();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_GripMovementReplicationType();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EGripMovementReplicationSettings();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_GripStiffness();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_IsHeld();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_IsInteractible();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnChildGrip();
	VREXPANSIONPLUGIN_API UScriptStruct* Z_Construct_UScriptStruct_FBPActorGripInformation();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnChildGripRelease();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnEndSecondaryUsed();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnEndUsed();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnGrip();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnGripRelease();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGrip();
	ENGINE_API UClass* Z_Construct_UClass_USceneComponent_NoRegister();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGripRelease();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnSecondaryUsed();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_OnUsed();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_ResetInitialLeverLocation();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_SecondaryGripType();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_ESecondaryGripType();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_SetHeld();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_SimulateOnDrop();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_SlotGripType();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_TeleportBehavior();
	VREXPANSIONPLUGIN_API UEnum* Z_Construct_UEnum_VRExpansionPlugin_EGripInterfaceTeleportBehavior();
	VREXPANSIONPLUGIN_API UFunction* Z_Construct_UFunction_UVRLeverComponent_TickGrip();
	VREXPANSIONPLUGIN_API UClass* Z_Construct_UClass_UVRLeverComponent_NoRegister();
	ENGINE_API UClass* Z_Construct_UClass_UStaticMeshComponent();
	GAMEPLAYTAGS_API UScriptStruct* Z_Construct_UScriptStruct_FGameplayTagContainer();
	VREXPANSIONPLUGIN_API UClass* Z_Construct_UClass_UVRGripInterface_NoRegister();
	GAMEPLAYTAGS_API UClass* Z_Construct_UClass_UGameplayTagAssetInterface_NoRegister();
// End Cross Module References
	UFunction* Z_Construct_UDelegateFunction_VRExpansionPlugin_VRLeverStateChangedSignature__DelegateSignature()
	{
		struct _Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms
		{
			bool LeverStatus;
			EVRInteractibleLeverEventType LeverStatusType;
			float LeverAngleAtTime;
		};
		UObject* Outer = Z_Construct_UPackage__Script_VRExpansionPlugin();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("VRLeverStateChangedSignature__DelegateSignature"), RF_Public|RF_Transient|RF_MarkAsNative) UDelegateFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x00130000, 65535, sizeof(_Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms));
			UProperty* NewProp_LeverAngleAtTime = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("LeverAngleAtTime"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(LeverAngleAtTime, _Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms), 0x0010000000000080);
			UProperty* NewProp_LeverStatusType = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("LeverStatusType"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(LeverStatusType, _Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms), 0x0010000000000080, Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType());
			UProperty* NewProp_LeverStatusType_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_LeverStatusType, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(LeverStatus, _Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms);
			UProperty* NewProp_LeverStatus = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("LeverStatus"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(LeverStatus, _Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms), 0x0010000000000080, CPP_BOOL_PROPERTY_BITMASK(LeverStatus, _Script_VRExpansionPlugin_eventVRLeverStateChangedSignature_Parms), sizeof(bool), true);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Delegate for notification when the lever state changes."));
#endif
		}
		return ReturnFunction;
	}
static UEnum* EVRInteractibleLeverReturnType_StaticEnum()
{
	static UEnum* Singleton = nullptr;
	if (!Singleton)
	{
		Singleton = GetStaticEnum(Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType, Z_Construct_UPackage__Script_VRExpansionPlugin(), TEXT("EVRInteractibleLeverReturnType"));
	}
	return Singleton;
}
static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EVRInteractibleLeverReturnType(EVRInteractibleLeverReturnType_StaticEnum, TEXT("/Script/VRExpansionPlugin"), TEXT("EVRInteractibleLeverReturnType"), false, nullptr, nullptr);
	UEnum* Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType()
	{
		UPackage* Outer = Z_Construct_UPackage__Script_VRExpansionPlugin();
		extern uint32 Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType_CRC();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EVRInteractibleLeverReturnType"), 0, Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType_CRC(), false);
		if (!ReturnEnum)
		{
			ReturnEnum = new(EC_InternalUseOnlyConstructor, Outer, TEXT("EVRInteractibleLeverReturnType"), RF_Public|RF_Transient|RF_MarkAsNative) UEnum(FObjectInitializer());
			TArray<TPair<FName, int64>> EnumNames;
			EnumNames.Emplace(TEXT("EVRInteractibleLeverReturnType::Stay"), 0);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverReturnType::ReturnToZero"), 1);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverReturnType::LerpToMax"), 2);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverReturnType::LerpToMaxIfOverThreshold"), 3);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverReturnType::EVRInteractibleLeverReturnType_MAX"), 4);
			ReturnEnum->SetEnums(EnumNames, UEnum::ECppForm::EnumClass);
			ReturnEnum->CppType = TEXT("EVRInteractibleLeverReturnType");
#if WITH_METADATA
			UMetaData* MetaData = ReturnEnum->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnEnum, TEXT("BlueprintType"), TEXT("true"));
			MetaData->SetValue(ReturnEnum, TEXT("IsBlueprintBase"), TEXT("true"));
			MetaData->SetValue(ReturnEnum, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
#endif
		}
		return ReturnEnum;
	}
	uint32 Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType_CRC() { return 3610291224U; }
static UEnum* EVRInteractibleLeverEventType_StaticEnum()
{
	static UEnum* Singleton = nullptr;
	if (!Singleton)
	{
		Singleton = GetStaticEnum(Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType, Z_Construct_UPackage__Script_VRExpansionPlugin(), TEXT("EVRInteractibleLeverEventType"));
	}
	return Singleton;
}
static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EVRInteractibleLeverEventType(EVRInteractibleLeverEventType_StaticEnum, TEXT("/Script/VRExpansionPlugin"), TEXT("EVRInteractibleLeverEventType"), false, nullptr, nullptr);
	UEnum* Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType()
	{
		UPackage* Outer = Z_Construct_UPackage__Script_VRExpansionPlugin();
		extern uint32 Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType_CRC();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EVRInteractibleLeverEventType"), 0, Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType_CRC(), false);
		if (!ReturnEnum)
		{
			ReturnEnum = new(EC_InternalUseOnlyConstructor, Outer, TEXT("EVRInteractibleLeverEventType"), RF_Public|RF_Transient|RF_MarkAsNative) UEnum(FObjectInitializer());
			TArray<TPair<FName, int64>> EnumNames;
			EnumNames.Emplace(TEXT("EVRInteractibleLeverEventType::LeverPositive"), 0);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverEventType::LeverNegative"), 1);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverEventType::EVRInteractibleLeverEventType_MAX"), 2);
			ReturnEnum->SetEnums(EnumNames, UEnum::ECppForm::EnumClass);
			ReturnEnum->CppType = TEXT("EVRInteractibleLeverEventType");
#if WITH_METADATA
			UMetaData* MetaData = ReturnEnum->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnEnum, TEXT("BlueprintType"), TEXT("true"));
			MetaData->SetValue(ReturnEnum, TEXT("IsBlueprintBase"), TEXT("true"));
			MetaData->SetValue(ReturnEnum, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
#endif
		}
		return ReturnEnum;
	}
	uint32 Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverEventType_CRC() { return 1155984870U; }
static UEnum* EVRInteractibleLeverAxis_StaticEnum()
{
	static UEnum* Singleton = nullptr;
	if (!Singleton)
	{
		Singleton = GetStaticEnum(Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis, Z_Construct_UPackage__Script_VRExpansionPlugin(), TEXT("EVRInteractibleLeverAxis"));
	}
	return Singleton;
}
static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EVRInteractibleLeverAxis(EVRInteractibleLeverAxis_StaticEnum, TEXT("/Script/VRExpansionPlugin"), TEXT("EVRInteractibleLeverAxis"), false, nullptr, nullptr);
	UEnum* Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis()
	{
		UPackage* Outer = Z_Construct_UPackage__Script_VRExpansionPlugin();
		extern uint32 Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis_CRC();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EVRInteractibleLeverAxis"), 0, Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis_CRC(), false);
		if (!ReturnEnum)
		{
			ReturnEnum = new(EC_InternalUseOnlyConstructor, Outer, TEXT("EVRInteractibleLeverAxis"), RF_Public|RF_Transient|RF_MarkAsNative) UEnum(FObjectInitializer());
			TArray<TPair<FName, int64>> EnumNames;
			EnumNames.Emplace(TEXT("EVRInteractibleLeverAxis::Axis_X"), 0);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverAxis::Axis_Y"), 1);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverAxis::Axis_XY"), 2);
			EnumNames.Emplace(TEXT("EVRInteractibleLeverAxis::Axis_MAX"), 3);
			ReturnEnum->SetEnums(EnumNames, UEnum::ECppForm::EnumClass);
			ReturnEnum->CppType = TEXT("EVRInteractibleLeverAxis");
#if WITH_METADATA
			UMetaData* MetaData = ReturnEnum->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnEnum, TEXT("BlueprintType"), TEXT("true"));
			MetaData->SetValue(ReturnEnum, TEXT("IsBlueprintBase"), TEXT("true"));
			MetaData->SetValue(ReturnEnum, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
#endif
		}
		return ReturnEnum;
	}
	uint32 Get_Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis_CRC() { return 3857322061U; }
	static FName NAME_UVRLeverComponent_AdvancedPhysicsSettings = FName(TEXT("AdvancedPhysicsSettings"));
	FBPAdvGripPhysicsSettings UVRLeverComponent::AdvancedPhysicsSettings()
	{
		VRLeverComponent_eventAdvancedPhysicsSettings_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_AdvancedPhysicsSettings),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_ClosestPrimarySlotInRange = FName(TEXT("ClosestPrimarySlotInRange"));
	void UVRLeverComponent::ClosestPrimarySlotInRange(FVector WorldLocation, bool& bHadSlotInRange, FTransform& SlotWorldTransform, UGripMotionControllerComponent* CallingController, FName OverridePrefix)
	{
		VRLeverComponent_eventClosestPrimarySlotInRange_Parms Parms;
		Parms.WorldLocation=WorldLocation;
		Parms.bHadSlotInRange=bHadSlotInRange ? true : false;
		Parms.SlotWorldTransform=SlotWorldTransform;
		Parms.CallingController=CallingController;
		Parms.OverridePrefix=OverridePrefix;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_ClosestPrimarySlotInRange),&Parms);
		bHadSlotInRange=Parms.bHadSlotInRange;
		SlotWorldTransform=Parms.SlotWorldTransform;
	}
	static FName NAME_UVRLeverComponent_ClosestSecondarySlotInRange = FName(TEXT("ClosestSecondarySlotInRange"));
	void UVRLeverComponent::ClosestSecondarySlotInRange(FVector WorldLocation, bool& bHadSlotInRange, FTransform& SlotWorldTransform, UGripMotionControllerComponent* CallingController, FName OverridePrefix)
	{
		VRLeverComponent_eventClosestSecondarySlotInRange_Parms Parms;
		Parms.WorldLocation=WorldLocation;
		Parms.bHadSlotInRange=bHadSlotInRange ? true : false;
		Parms.SlotWorldTransform=SlotWorldTransform;
		Parms.CallingController=CallingController;
		Parms.OverridePrefix=OverridePrefix;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_ClosestSecondarySlotInRange),&Parms);
		bHadSlotInRange=Parms.bHadSlotInRange;
		SlotWorldTransform=Parms.SlotWorldTransform;
	}
	static FName NAME_UVRLeverComponent_DenyGripping = FName(TEXT("DenyGripping"));
	bool UVRLeverComponent::DenyGripping()
	{
		VRLeverComponent_eventDenyGripping_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_DenyGripping),&Parms);
		return !!Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_FreeGripType = FName(TEXT("FreeGripType"));
	EGripCollisionType UVRLeverComponent::FreeGripType()
	{
		VRLeverComponent_eventFreeGripType_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_FreeGripType),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_GetInteractionSettings = FName(TEXT("GetInteractionSettings"));
	FBPInteractionSettings UVRLeverComponent::GetInteractionSettings()
	{
		VRLeverComponent_eventGetInteractionSettings_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_GetInteractionSettings),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_GripBreakDistance = FName(TEXT("GripBreakDistance"));
	float UVRLeverComponent::GripBreakDistance()
	{
		VRLeverComponent_eventGripBreakDistance_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_GripBreakDistance),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_GripDamping = FName(TEXT("GripDamping"));
	float UVRLeverComponent::GripDamping()
	{
		VRLeverComponent_eventGripDamping_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_GripDamping),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_GripLateUpdateSetting = FName(TEXT("GripLateUpdateSetting"));
	EGripLateUpdateSettings UVRLeverComponent::GripLateUpdateSetting()
	{
		VRLeverComponent_eventGripLateUpdateSetting_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_GripLateUpdateSetting),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_GripMovementReplicationType = FName(TEXT("GripMovementReplicationType"));
	EGripMovementReplicationSettings UVRLeverComponent::GripMovementReplicationType()
	{
		VRLeverComponent_eventGripMovementReplicationType_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_GripMovementReplicationType),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_GripStiffness = FName(TEXT("GripStiffness"));
	float UVRLeverComponent::GripStiffness()
	{
		VRLeverComponent_eventGripStiffness_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_GripStiffness),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_IsHeld = FName(TEXT("IsHeld"));
	void UVRLeverComponent::IsHeld(UGripMotionControllerComponent*& CurHoldingController, bool& bCurIsHeld)
	{
		VRLeverComponent_eventIsHeld_Parms Parms;
		Parms.CurHoldingController=CurHoldingController;
		Parms.bCurIsHeld=bCurIsHeld ? true : false;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_IsHeld),&Parms);
		CurHoldingController=Parms.CurHoldingController;
		bCurIsHeld=Parms.bCurIsHeld;
	}
	static FName NAME_UVRLeverComponent_IsInteractible = FName(TEXT("IsInteractible"));
	bool UVRLeverComponent::IsInteractible()
	{
		VRLeverComponent_eventIsInteractible_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_IsInteractible),&Parms);
		return !!Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_OnChildGrip = FName(TEXT("OnChildGrip"));
	void UVRLeverComponent::OnChildGrip(UGripMotionControllerComponent* GrippingController, FBPActorGripInformation const& GripInformation)
	{
		VRLeverComponent_eventOnChildGrip_Parms Parms;
		Parms.GrippingController=GrippingController;
		Parms.GripInformation=GripInformation;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnChildGrip),&Parms);
	}
	static FName NAME_UVRLeverComponent_OnChildGripRelease = FName(TEXT("OnChildGripRelease"));
	void UVRLeverComponent::OnChildGripRelease(UGripMotionControllerComponent* ReleasingController, FBPActorGripInformation const& GripInformation)
	{
		VRLeverComponent_eventOnChildGripRelease_Parms Parms;
		Parms.ReleasingController=ReleasingController;
		Parms.GripInformation=GripInformation;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnChildGripRelease),&Parms);
	}
	static FName NAME_UVRLeverComponent_OnEndSecondaryUsed = FName(TEXT("OnEndSecondaryUsed"));
	void UVRLeverComponent::OnEndSecondaryUsed()
	{
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnEndSecondaryUsed),NULL);
	}
	static FName NAME_UVRLeverComponent_OnEndUsed = FName(TEXT("OnEndUsed"));
	void UVRLeverComponent::OnEndUsed()
	{
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnEndUsed),NULL);
	}
	static FName NAME_UVRLeverComponent_OnGrip = FName(TEXT("OnGrip"));
	void UVRLeverComponent::OnGrip(UGripMotionControllerComponent* GrippingController, FBPActorGripInformation const& GripInformation)
	{
		VRLeverComponent_eventOnGrip_Parms Parms;
		Parms.GrippingController=GrippingController;
		Parms.GripInformation=GripInformation;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnGrip),&Parms);
	}
	static FName NAME_UVRLeverComponent_OnGripRelease = FName(TEXT("OnGripRelease"));
	void UVRLeverComponent::OnGripRelease(UGripMotionControllerComponent* ReleasingController, FBPActorGripInformation const& GripInformation)
	{
		VRLeverComponent_eventOnGripRelease_Parms Parms;
		Parms.ReleasingController=ReleasingController;
		Parms.GripInformation=GripInformation;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnGripRelease),&Parms);
	}
	static FName NAME_UVRLeverComponent_OnSecondaryGrip = FName(TEXT("OnSecondaryGrip"));
	void UVRLeverComponent::OnSecondaryGrip(USceneComponent* SecondaryGripComponent, FBPActorGripInformation const& GripInformation)
	{
		VRLeverComponent_eventOnSecondaryGrip_Parms Parms;
		Parms.SecondaryGripComponent=SecondaryGripComponent;
		Parms.GripInformation=GripInformation;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnSecondaryGrip),&Parms);
	}
	static FName NAME_UVRLeverComponent_OnSecondaryGripRelease = FName(TEXT("OnSecondaryGripRelease"));
	void UVRLeverComponent::OnSecondaryGripRelease(USceneComponent* ReleasingSecondaryGripComponent, FBPActorGripInformation const& GripInformation)
	{
		VRLeverComponent_eventOnSecondaryGripRelease_Parms Parms;
		Parms.ReleasingSecondaryGripComponent=ReleasingSecondaryGripComponent;
		Parms.GripInformation=GripInformation;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnSecondaryGripRelease),&Parms);
	}
	static FName NAME_UVRLeverComponent_OnSecondaryUsed = FName(TEXT("OnSecondaryUsed"));
	void UVRLeverComponent::OnSecondaryUsed()
	{
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnSecondaryUsed),NULL);
	}
	static FName NAME_UVRLeverComponent_OnUsed = FName(TEXT("OnUsed"));
	void UVRLeverComponent::OnUsed()
	{
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_OnUsed),NULL);
	}
	static FName NAME_UVRLeverComponent_SecondaryGripType = FName(TEXT("SecondaryGripType"));
	ESecondaryGripType UVRLeverComponent::SecondaryGripType()
	{
		VRLeverComponent_eventSecondaryGripType_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_SecondaryGripType),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_SetHeld = FName(TEXT("SetHeld"));
	void UVRLeverComponent::SetHeld(UGripMotionControllerComponent* NewHoldingController, bool bNewIsHeld)
	{
		VRLeverComponent_eventSetHeld_Parms Parms;
		Parms.NewHoldingController=NewHoldingController;
		Parms.bNewIsHeld=bNewIsHeld ? true : false;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_SetHeld),&Parms);
	}
	static FName NAME_UVRLeverComponent_SimulateOnDrop = FName(TEXT("SimulateOnDrop"));
	bool UVRLeverComponent::SimulateOnDrop()
	{
		VRLeverComponent_eventSimulateOnDrop_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_SimulateOnDrop),&Parms);
		return !!Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_SlotGripType = FName(TEXT("SlotGripType"));
	EGripCollisionType UVRLeverComponent::SlotGripType()
	{
		VRLeverComponent_eventSlotGripType_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_SlotGripType),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_TeleportBehavior = FName(TEXT("TeleportBehavior"));
	EGripInterfaceTeleportBehavior UVRLeverComponent::TeleportBehavior()
	{
		VRLeverComponent_eventTeleportBehavior_Parms Parms;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_TeleportBehavior),&Parms);
		return Parms.ReturnValue;
	}
	static FName NAME_UVRLeverComponent_TickGrip = FName(TEXT("TickGrip"));
	void UVRLeverComponent::TickGrip(UGripMotionControllerComponent* GrippingController, FBPActorGripInformation const& GripInformation, float DeltaTime)
	{
		VRLeverComponent_eventTickGrip_Parms Parms;
		Parms.GrippingController=GrippingController;
		Parms.GripInformation=GripInformation;
		Parms.DeltaTime=DeltaTime;
		ProcessEvent(FindFunctionChecked(NAME_UVRLeverComponent_TickGrip),&Parms);
	}
	void UVRLeverComponent::StaticRegisterNativesUVRLeverComponent()
	{
		UClass* Class = UVRLeverComponent::StaticClass();
		static const TNameNativePtrPair<ANSICHAR> AnsiFuncs[] = {
			{ "AdvancedPhysicsSettings", (Native)&UVRLeverComponent::execAdvancedPhysicsSettings },
			{ "ClosestPrimarySlotInRange", (Native)&UVRLeverComponent::execClosestPrimarySlotInRange },
			{ "ClosestSecondarySlotInRange", (Native)&UVRLeverComponent::execClosestSecondarySlotInRange },
			{ "DenyGripping", (Native)&UVRLeverComponent::execDenyGripping },
			{ "FreeGripType", (Native)&UVRLeverComponent::execFreeGripType },
			{ "GetInteractionSettings", (Native)&UVRLeverComponent::execGetInteractionSettings },
			{ "GripBreakDistance", (Native)&UVRLeverComponent::execGripBreakDistance },
			{ "GripDamping", (Native)&UVRLeverComponent::execGripDamping },
			{ "GripLateUpdateSetting", (Native)&UVRLeverComponent::execGripLateUpdateSetting },
			{ "GripMovementReplicationType", (Native)&UVRLeverComponent::execGripMovementReplicationType },
			{ "GripStiffness", (Native)&UVRLeverComponent::execGripStiffness },
			{ "IsHeld", (Native)&UVRLeverComponent::execIsHeld },
			{ "IsInteractible", (Native)&UVRLeverComponent::execIsInteractible },
			{ "OnChildGrip", (Native)&UVRLeverComponent::execOnChildGrip },
			{ "OnChildGripRelease", (Native)&UVRLeverComponent::execOnChildGripRelease },
			{ "OnEndSecondaryUsed", (Native)&UVRLeverComponent::execOnEndSecondaryUsed },
			{ "OnEndUsed", (Native)&UVRLeverComponent::execOnEndUsed },
			{ "OnGrip", (Native)&UVRLeverComponent::execOnGrip },
			{ "OnGripRelease", (Native)&UVRLeverComponent::execOnGripRelease },
			{ "OnSecondaryGrip", (Native)&UVRLeverComponent::execOnSecondaryGrip },
			{ "OnSecondaryGripRelease", (Native)&UVRLeverComponent::execOnSecondaryGripRelease },
			{ "OnSecondaryUsed", (Native)&UVRLeverComponent::execOnSecondaryUsed },
			{ "OnUsed", (Native)&UVRLeverComponent::execOnUsed },
			{ "ResetInitialLeverLocation", (Native)&UVRLeverComponent::execResetInitialLeverLocation },
			{ "SecondaryGripType", (Native)&UVRLeverComponent::execSecondaryGripType },
			{ "SetHeld", (Native)&UVRLeverComponent::execSetHeld },
			{ "SimulateOnDrop", (Native)&UVRLeverComponent::execSimulateOnDrop },
			{ "SlotGripType", (Native)&UVRLeverComponent::execSlotGripType },
			{ "TeleportBehavior", (Native)&UVRLeverComponent::execTeleportBehavior },
			{ "TickGrip", (Native)&UVRLeverComponent::execTickGrip },
		};
		FNativeFunctionRegistrar::RegisterFunctions(Class, AnsiFuncs, ARRAY_COUNT(AnsiFuncs));
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_AdvancedPhysicsSettings()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("AdvancedPhysicsSettings"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventAdvancedPhysicsSettings_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventAdvancedPhysicsSettings_Parms), 0x0010000000000580, Z_Construct_UScriptStruct_FBPAdvGripPhysicsSettings());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Get the advanced physics settings for this grip"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_ClosestPrimarySlotInRange()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("ClosestPrimarySlotInRange"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0CC20C00, 65535, sizeof(VRLeverComponent_eventClosestPrimarySlotInRange_Parms));
			UProperty* NewProp_OverridePrefix = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("OverridePrefix"), RF_Public|RF_Transient|RF_MarkAsNative) UNameProperty(CPP_PROPERTY_BASE(OverridePrefix, VRLeverComponent_eventClosestPrimarySlotInRange_Parms), 0x0010000000000080);
			UProperty* NewProp_CallingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("CallingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(CallingController, VRLeverComponent_eventClosestPrimarySlotInRange_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			UProperty* NewProp_SlotWorldTransform = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("SlotWorldTransform"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(SlotWorldTransform, VRLeverComponent_eventClosestPrimarySlotInRange_Parms), 0x0010000000000180, Z_Construct_UScriptStruct_FTransform());
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(bHadSlotInRange, VRLeverComponent_eventClosestPrimarySlotInRange_Parms);
			UProperty* NewProp_bHadSlotInRange = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("bHadSlotInRange"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bHadSlotInRange, VRLeverComponent_eventClosestPrimarySlotInRange_Parms), 0x0010000000000180, CPP_BOOL_PROPERTY_BITMASK(bHadSlotInRange, VRLeverComponent_eventClosestPrimarySlotInRange_Parms), sizeof(bool), true);
			UProperty* NewProp_WorldLocation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("WorldLocation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(WorldLocation, VRLeverComponent_eventClosestPrimarySlotInRange_Parms), 0x0010000000000080, Z_Construct_UScriptStruct_FVector());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("CPP_Default_OverridePrefix"), TEXT("None"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Get closest primary slot in range"));
			MetaData->SetValue(NewProp_CallingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_ClosestSecondarySlotInRange()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("ClosestSecondarySlotInRange"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0CC20C00, 65535, sizeof(VRLeverComponent_eventClosestSecondarySlotInRange_Parms));
			UProperty* NewProp_OverridePrefix = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("OverridePrefix"), RF_Public|RF_Transient|RF_MarkAsNative) UNameProperty(CPP_PROPERTY_BASE(OverridePrefix, VRLeverComponent_eventClosestSecondarySlotInRange_Parms), 0x0010000000000080);
			UProperty* NewProp_CallingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("CallingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(CallingController, VRLeverComponent_eventClosestSecondarySlotInRange_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			UProperty* NewProp_SlotWorldTransform = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("SlotWorldTransform"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(SlotWorldTransform, VRLeverComponent_eventClosestSecondarySlotInRange_Parms), 0x0010000000000180, Z_Construct_UScriptStruct_FTransform());
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(bHadSlotInRange, VRLeverComponent_eventClosestSecondarySlotInRange_Parms);
			UProperty* NewProp_bHadSlotInRange = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("bHadSlotInRange"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bHadSlotInRange, VRLeverComponent_eventClosestSecondarySlotInRange_Parms), 0x0010000000000180, CPP_BOOL_PROPERTY_BITMASK(bHadSlotInRange, VRLeverComponent_eventClosestSecondarySlotInRange_Parms), sizeof(bool), true);
			UProperty* NewProp_WorldLocation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("WorldLocation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(WorldLocation, VRLeverComponent_eventClosestSecondarySlotInRange_Parms), 0x0010000000000080, Z_Construct_UScriptStruct_FVector());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("CPP_Default_OverridePrefix"), TEXT("None"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Get closest secondary slot in range"));
			MetaData->SetValue(NewProp_CallingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_DenyGripping()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("DenyGripping"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventDenyGripping_Parms));
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(ReturnValue, VRLeverComponent_eventDenyGripping_Parms);
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(ReturnValue, VRLeverComponent_eventDenyGripping_Parms), 0x0010000000000580, CPP_BOOL_PROPERTY_BITMASK(ReturnValue, VRLeverComponent_eventDenyGripping_Parms), sizeof(bool), true);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Set up as deny instead of allow so that default allows for gripping"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_FreeGripType()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("FreeGripType"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventFreeGripType_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventFreeGripType_Parms), 0x0010000000000580, Z_Construct_UEnum_VRExpansionPlugin_EGripCollisionType());
			UProperty* NewProp_ReturnValue_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_ReturnValue, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Grip type to use when not gripping a slot"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_GetInteractionSettings()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("GetInteractionSettings"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventGetInteractionSettings_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventGetInteractionSettings_Parms), 0x0010000000000580, Z_Construct_UScriptStruct_FBPInteractionSettings());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Get interactable settings"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_GripBreakDistance()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("GripBreakDistance"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventGripBreakDistance_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventGripBreakDistance_Parms), 0x0010000000000580);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("What distance to break a grip at (only relevent with physics enabled grips"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_GripDamping()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("GripDamping"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventGripDamping_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventGripDamping_Parms), 0x0010000000000580);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("What grip damping to use if using a physics constraint"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_GripLateUpdateSetting()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("GripLateUpdateSetting"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventGripLateUpdateSetting_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventGripLateUpdateSetting_Parms), 0x0010000000000580, Z_Construct_UEnum_VRExpansionPlugin_EGripLateUpdateSettings());
			UProperty* NewProp_ReturnValue_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_ReturnValue, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Define the late update setting"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_GripMovementReplicationType()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("GripMovementReplicationType"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventGripMovementReplicationType_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventGripMovementReplicationType_Parms), 0x0010000000000580, Z_Construct_UEnum_VRExpansionPlugin_EGripMovementReplicationSettings());
			UProperty* NewProp_ReturnValue_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_ReturnValue, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Define which movement repliation setting to use"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_GripStiffness()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("GripStiffness"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventGripStiffness_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventGripStiffness_Parms), 0x0010000000000580);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("What grip stiffness to use if using a physics constraint"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_IsHeld()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("IsHeld"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C420C00, 65535, sizeof(VRLeverComponent_eventIsHeld_Parms));
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(bCurIsHeld, VRLeverComponent_eventIsHeld_Parms);
			UProperty* NewProp_bCurIsHeld = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("bCurIsHeld"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bCurIsHeld, VRLeverComponent_eventIsHeld_Parms), 0x0010000000000180, CPP_BOOL_PROPERTY_BITMASK(bCurIsHeld, VRLeverComponent_eventIsHeld_Parms), sizeof(bool), true);
			UProperty* NewProp_CurHoldingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("CurHoldingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(CurHoldingController, VRLeverComponent_eventIsHeld_Parms), 0x0010000000080180, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Returns if the object is held and if so, which pawn is holding it"));
			MetaData->SetValue(NewProp_CurHoldingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_IsInteractible()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("IsInteractible"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventIsInteractible_Parms));
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(ReturnValue, VRLeverComponent_eventIsInteractible_Parms);
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(ReturnValue, VRLeverComponent_eventIsInteractible_Parms), 0x0010000000000580, CPP_BOOL_PROPERTY_BITMASK(ReturnValue, VRLeverComponent_eventIsInteractible_Parms), sizeof(bool), true);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Check if the object is an interactable"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnChildGrip()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnChildGrip"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventOnChildGrip_Parms));
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventOnChildGrip_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_GrippingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GrippingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(GrippingController, VRLeverComponent_eventOnChildGrip_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered on the interfaced object when child component is gripped"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_GrippingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnChildGripRelease()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnChildGripRelease"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventOnChildGripRelease_Parms));
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventOnChildGripRelease_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_ReleasingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReleasingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(ReleasingController, VRLeverComponent_eventOnChildGripRelease_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered on the interfaced object when child component is released"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_ReleasingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnEndSecondaryUsed()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnEndSecondaryUsed"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Call to stop using an object"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnEndUsed()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnEndUsed"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Call to stop using an object"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnGrip()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnGrip"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventOnGrip_Parms));
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventOnGrip_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_GrippingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GrippingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(GrippingController, VRLeverComponent_eventOnGrip_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered on the interfaced object when gripped"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_GrippingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnGripRelease()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnGripRelease"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventOnGripRelease_Parms));
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventOnGripRelease_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_ReleasingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReleasingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(ReleasingController, VRLeverComponent_eventOnGripRelease_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered on the interfaced object when grip is released"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_ReleasingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGrip()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnSecondaryGrip"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventOnSecondaryGrip_Parms));
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventOnSecondaryGrip_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_SecondaryGripComponent = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("SecondaryGripComponent"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(SecondaryGripComponent, VRLeverComponent_eventOnSecondaryGrip_Parms), 0x0010000000080080, Z_Construct_UClass_USceneComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered on the interfaced object when secondary gripped"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_SecondaryGripComponent, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGripRelease()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnSecondaryGripRelease"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventOnSecondaryGripRelease_Parms));
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventOnSecondaryGripRelease_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_ReleasingSecondaryGripComponent = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReleasingSecondaryGripComponent"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(ReleasingSecondaryGripComponent, VRLeverComponent_eventOnSecondaryGripRelease_Parms), 0x0010000000080080, Z_Construct_UClass_USceneComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered on the interfaced object when secondary grip is released"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_ReleasingSecondaryGripComponent, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnSecondaryUsed()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnSecondaryUsed"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Call to use an object"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_OnUsed()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("OnUsed"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Call to use an object"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_ResetInitialLeverLocation()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("ResetInitialLeverLocation"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x04020401, 65535);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRLeverComponent"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Should be called after the lever is moved post begin play"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_SecondaryGripType()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("SecondaryGripType"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventSecondaryGripType_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventSecondaryGripType_Parms), 0x0010000000000580, Z_Construct_UEnum_VRExpansionPlugin_ESecondaryGripType());
			UProperty* NewProp_ReturnValue_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_ReturnValue, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Secondary grip type"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_SetHeld()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("SetHeld"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventSetHeld_Parms));
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(bNewIsHeld, VRLeverComponent_eventSetHeld_Parms);
			UProperty* NewProp_bNewIsHeld = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("bNewIsHeld"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bNewIsHeld, VRLeverComponent_eventSetHeld_Parms), 0x0010000000000080, CPP_BOOL_PROPERTY_BITMASK(bNewIsHeld, VRLeverComponent_eventSetHeld_Parms), sizeof(bool), true);
			UProperty* NewProp_NewHoldingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("NewHoldingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(NewHoldingController, VRLeverComponent_eventSetHeld_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Sets is held, used by the plugin"));
			MetaData->SetValue(NewProp_NewHoldingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_SimulateOnDrop()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("SimulateOnDrop"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventSimulateOnDrop_Parms));
			CPP_BOOL_PROPERTY_BITMASK_STRUCT(ReturnValue, VRLeverComponent_eventSimulateOnDrop_Parms);
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(ReturnValue, VRLeverComponent_eventSimulateOnDrop_Parms), 0x0010000000000580, CPP_BOOL_PROPERTY_BITMASK(ReturnValue, VRLeverComponent_eventSimulateOnDrop_Parms), sizeof(bool), true);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Should this object simulate on drop"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_SlotGripType()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("SlotGripType"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventSlotGripType_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventSlotGripType_Parms), 0x0010000000000580, Z_Construct_UEnum_VRExpansionPlugin_EGripCollisionType());
			UProperty* NewProp_ReturnValue_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_ReturnValue, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Grip type to use when gripping a slot"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_TeleportBehavior()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("TeleportBehavior"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x0C020C00, 65535, sizeof(VRLeverComponent_eventTeleportBehavior_Parms));
			UProperty* NewProp_ReturnValue = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("ReturnValue"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(ReturnValue, VRLeverComponent_eventTeleportBehavior_Parms), 0x0010000000000580, Z_Construct_UEnum_VRExpansionPlugin_EGripInterfaceTeleportBehavior());
			UProperty* NewProp_ReturnValue_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_ReturnValue, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("How an interfaced object behaves when teleporting"));
#endif
		}
		return ReturnFunction;
	}
	UFunction* Z_Construct_UFunction_UVRLeverComponent_TickGrip()
	{
		UObject* Outer = Z_Construct_UClass_UVRLeverComponent();
		static UFunction* ReturnFunction = nullptr;
		if (!ReturnFunction)
		{
			ReturnFunction = new(EC_InternalUseOnlyConstructor, Outer, TEXT("TickGrip"), RF_Public|RF_Transient|RF_MarkAsNative) UFunction(FObjectInitializer(), nullptr, (EFunctionFlags)0x08420C00, 65535, sizeof(VRLeverComponent_eventTickGrip_Parms));
			UProperty* NewProp_DeltaTime = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("DeltaTime"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(DeltaTime, VRLeverComponent_eventTickGrip_Parms), 0x0010000000000080);
			UProperty* NewProp_GripInformation = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GripInformation"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GripInformation, VRLeverComponent_eventTickGrip_Parms), 0x0010008008000182, Z_Construct_UScriptStruct_FBPActorGripInformation());
			UProperty* NewProp_GrippingController = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("GrippingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(GrippingController, VRLeverComponent_eventTickGrip_Parms), 0x0010000000080080, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
			ReturnFunction->Bind();
			ReturnFunction->StaticLink();
#if WITH_METADATA
			UMetaData* MetaData = ReturnFunction->GetOutermost()->GetMetaData();
			MetaData->SetValue(ReturnFunction, TEXT("Category"), TEXT("VRGripInterface"));
			MetaData->SetValue(ReturnFunction, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
			MetaData->SetValue(ReturnFunction, TEXT("ToolTip"), TEXT("Event triggered each tick on the interfaced object when gripped, can be used for custom movement or grip based logic"));
			MetaData->SetValue(NewProp_GripInformation, TEXT("NativeConst"), TEXT(""));
			MetaData->SetValue(NewProp_GrippingController, TEXT("EditInline"), TEXT("true"));
#endif
		}
		return ReturnFunction;
	}
	UClass* Z_Construct_UClass_UVRLeverComponent_NoRegister()
	{
		return UVRLeverComponent::StaticClass();
	}
	UClass* Z_Construct_UClass_UVRLeverComponent()
	{
		static UClass* OuterClass = NULL;
		if (!OuterClass)
		{
			Z_Construct_UClass_UStaticMeshComponent();
			Z_Construct_UPackage__Script_VRExpansionPlugin();
			OuterClass = UVRLeverComponent::StaticClass();
			if (!(OuterClass->ClassFlags & CLASS_Constructed))
			{
				UObjectForceRegistration(OuterClass);
				OuterClass->ClassFlags |= (EClassFlags)0x20B01080u;

				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_AdvancedPhysicsSettings());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_ClosestPrimarySlotInRange());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_ClosestSecondarySlotInRange());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_DenyGripping());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_FreeGripType());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_GetInteractionSettings());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_GripBreakDistance());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_GripDamping());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_GripLateUpdateSetting());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_GripMovementReplicationType());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_GripStiffness());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_IsHeld());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_IsInteractible());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnChildGrip());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnChildGripRelease());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnEndSecondaryUsed());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnEndUsed());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnGrip());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnGripRelease());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGrip());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGripRelease());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnSecondaryUsed());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_OnUsed());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_ResetInitialLeverLocation());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_SecondaryGripType());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_SetHeld());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_SimulateOnDrop());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_SlotGripType());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_TeleportBehavior());
				OuterClass->LinkChild(Z_Construct_UFunction_UVRLeverComponent_TickGrip());

				UProperty* NewProp_HoldingController = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("HoldingController"), RF_Public|RF_Transient|RF_MarkAsNative) UObjectProperty(CPP_PROPERTY_BASE(HoldingController, UVRLeverComponent), 0x001000000008001c, Z_Construct_UClass_UGripMotionControllerComponent_NoRegister());
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bIsHeld, UVRLeverComponent);
				UProperty* NewProp_bIsHeld = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bIsHeld"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bIsHeld, UVRLeverComponent), 0x0010000000000014, CPP_BOOL_PROPERTY_BITMASK(bIsHeld, UVRLeverComponent), sizeof(bool), true);
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bDenyGripping, UVRLeverComponent);
				UProperty* NewProp_bDenyGripping = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bDenyGripping"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bDenyGripping, UVRLeverComponent), 0x0010000000000004, CPP_BOOL_PROPERTY_BITMASK(bDenyGripping, UVRLeverComponent), sizeof(bool), true);
				UProperty* NewProp_Damping = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("Damping"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(Damping, UVRLeverComponent), 0x0010000000000005);
				UProperty* NewProp_Stiffness = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("Stiffness"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(Stiffness, UVRLeverComponent), 0x0010000000000005);
				UProperty* NewProp_BreakDistance = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("BreakDistance"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(BreakDistance, UVRLeverComponent), 0x0010000000000005);
				UProperty* NewProp_MovementReplicationSetting = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("MovementReplicationSetting"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(MovementReplicationSetting, UVRLeverComponent), 0x0010000000000005, Z_Construct_UEnum_VRExpansionPlugin_EGripMovementReplicationSettings());
				UProperty* NewProp_MovementReplicationSetting_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_MovementReplicationSetting, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bReplicateMovement, UVRLeverComponent);
				UProperty* NewProp_bReplicateMovement = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bReplicateMovement"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bReplicateMovement, UVRLeverComponent), 0x0010000000000025, CPP_BOOL_PROPERTY_BITMASK(bReplicateMovement, UVRLeverComponent), sizeof(bool), true);
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bRepGameplayTags, UVRLeverComponent);
				UProperty* NewProp_bRepGameplayTags = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bRepGameplayTags"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bRepGameplayTags, UVRLeverComponent), 0x0010000000000025, CPP_BOOL_PROPERTY_BITMASK(bRepGameplayTags, UVRLeverComponent), sizeof(bool), true);
				UProperty* NewProp_GameplayTags = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("GameplayTags"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(GameplayTags, UVRLeverComponent), 0x0010000000000025, Z_Construct_UScriptStruct_FGameplayTagContainer());
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bIsLerping, UVRLeverComponent);
				UProperty* NewProp_bIsLerping = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bIsLerping"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bIsLerping, UVRLeverComponent), 0x0010000000000014, CPP_BOOL_PROPERTY_BITMASK(bIsLerping, UVRLeverComponent), sizeof(bool), true);
				UProperty* NewProp_LeverReturnSpeed = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("LeverReturnSpeed"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(LeverReturnSpeed, UVRLeverComponent), 0x0010000000000005);
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bSendLeverEventsDuringLerp, UVRLeverComponent);
				UProperty* NewProp_bSendLeverEventsDuringLerp = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bSendLeverEventsDuringLerp"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bSendLeverEventsDuringLerp, UVRLeverComponent), 0x0010000000000005, CPP_BOOL_PROPERTY_BITMASK(bSendLeverEventsDuringLerp, UVRLeverComponent), sizeof(bool), true);
				UProperty* NewProp_LeverReturnTypeWhenReleased = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("LeverReturnTypeWhenReleased"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(LeverReturnTypeWhenReleased, UVRLeverComponent), 0x0010000000000005, Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverReturnType());
				UProperty* NewProp_LeverReturnTypeWhenReleased_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_LeverReturnTypeWhenReleased, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
				UProperty* NewProp_LeverLimitNegative = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("LeverLimitNegative"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(LeverLimitNegative, UVRLeverComponent), 0x0010000000000005);
				UProperty* NewProp_LeverLimitPositive = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("LeverLimitPositive"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(LeverLimitPositive, UVRLeverComponent), 0x0010000000000005);
				UProperty* NewProp_LeverTogglePercentage = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("LeverTogglePercentage"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(LeverTogglePercentage, UVRLeverComponent), 0x0010000000000005);
				UProperty* NewProp_LeverRotationAxis = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("LeverRotationAxis"), RF_Public|RF_Transient|RF_MarkAsNative) UEnumProperty(CPP_PROPERTY_BASE(LeverRotationAxis, UVRLeverComponent), 0x0010000000000005, Z_Construct_UEnum_VRExpansionPlugin_EVRInteractibleLeverAxis());
				UProperty* NewProp_LeverRotationAxis_Underlying = new(EC_InternalUseOnlyConstructor, NewProp_LeverRotationAxis, TEXT("UnderlyingType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(FObjectInitializer(), EC_CppProperty, 0, 0x0000000000000000);
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bUngripAtTargetRotation, UVRLeverComponent);
				UProperty* NewProp_bUngripAtTargetRotation = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bUngripAtTargetRotation"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bUngripAtTargetRotation, UVRLeverComponent), 0x0010000000000005, CPP_BOOL_PROPERTY_BITMASK(bUngripAtTargetRotation, UVRLeverComponent), sizeof(bool), true);
				CPP_BOOL_PROPERTY_BITMASK_STRUCT(bIsPhysicsLever, UVRLeverComponent);
				UProperty* NewProp_bIsPhysicsLever = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("bIsPhysicsLever"), RF_Public|RF_Transient|RF_MarkAsNative) UBoolProperty(FObjectInitializer(), EC_CppProperty, CPP_BOOL_PROPERTY_OFFSET(bIsPhysicsLever, UVRLeverComponent), 0x0010000000000005, CPP_BOOL_PROPERTY_BITMASK(bIsPhysicsLever, UVRLeverComponent), sizeof(bool), true);
				UProperty* NewProp_CurrentLeverForwardVector = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("CurrentLeverForwardVector"), RF_Public|RF_Transient|RF_MarkAsNative) UStructProperty(CPP_PROPERTY_BASE(CurrentLeverForwardVector, UVRLeverComponent), 0x0010000000000014, Z_Construct_UScriptStruct_FVector());
				UProperty* NewProp_CurrentLeverAngle = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("CurrentLeverAngle"), RF_Public|RF_Transient|RF_MarkAsNative) UFloatProperty(CPP_PROPERTY_BASE(CurrentLeverAngle, UVRLeverComponent), 0x0010000000000014);
				UProperty* NewProp_OnLeverStateChanged = new(EC_InternalUseOnlyConstructor, OuterClass, TEXT("OnLeverStateChanged"), RF_Public|RF_Transient|RF_MarkAsNative) UMulticastDelegateProperty(CPP_PROPERTY_BASE(OnLeverStateChanged, UVRLeverComponent), 0x0010000010080000, Z_Construct_UDelegateFunction_VRExpansionPlugin_VRLeverStateChangedSignature__DelegateSignature());
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_AdvancedPhysicsSettings(), "AdvancedPhysicsSettings"); // 3347138668
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_ClosestPrimarySlotInRange(), "ClosestPrimarySlotInRange"); // 438060591
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_ClosestSecondarySlotInRange(), "ClosestSecondarySlotInRange"); // 1878248475
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_DenyGripping(), "DenyGripping"); // 2084318172
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_FreeGripType(), "FreeGripType"); // 2496679248
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_GetInteractionSettings(), "GetInteractionSettings"); // 3434696133
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_GripBreakDistance(), "GripBreakDistance"); // 4087518551
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_GripDamping(), "GripDamping"); // 2693862027
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_GripLateUpdateSetting(), "GripLateUpdateSetting"); // 2429717764
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_GripMovementReplicationType(), "GripMovementReplicationType"); // 767984755
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_GripStiffness(), "GripStiffness"); // 1052781627
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_IsHeld(), "IsHeld"); // 4074440413
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_IsInteractible(), "IsInteractible"); // 3406085146
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnChildGrip(), "OnChildGrip"); // 138330243
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnChildGripRelease(), "OnChildGripRelease"); // 1326153375
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnEndSecondaryUsed(), "OnEndSecondaryUsed"); // 2877808663
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnEndUsed(), "OnEndUsed"); // 3044231651
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnGrip(), "OnGrip"); // 2431762710
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnGripRelease(), "OnGripRelease"); // 3294151032
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGrip(), "OnSecondaryGrip"); // 3531990845
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnSecondaryGripRelease(), "OnSecondaryGripRelease"); // 1740706214
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnSecondaryUsed(), "OnSecondaryUsed"); // 4000345445
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_OnUsed(), "OnUsed"); // 3451834395
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_ResetInitialLeverLocation(), "ResetInitialLeverLocation"); // 2026741612
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_SecondaryGripType(), "SecondaryGripType"); // 4088505386
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_SetHeld(), "SetHeld"); // 1545792897
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_SimulateOnDrop(), "SimulateOnDrop"); // 3959803093
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_SlotGripType(), "SlotGripType"); // 271971734
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_TeleportBehavior(), "TeleportBehavior"); // 3447102280
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UVRLeverComponent_TickGrip(), "TickGrip"); // 3128668699
				static TCppClassTypeInfo<TCppClassTypeTraits<UVRLeverComponent> > StaticCppClassTypeInfo;
				OuterClass->SetCppTypeInfo(&StaticCppClassTypeInfo);
				OuterClass->Interfaces.Add(FImplementedInterface(Z_Construct_UClass_UVRGripInterface_NoRegister(), VTABLE_OFFSET(UVRLeverComponent, IVRGripInterface), false ));
				OuterClass->Interfaces.Add(FImplementedInterface(Z_Construct_UClass_UGameplayTagAssetInterface_NoRegister(), VTABLE_OFFSET(UVRLeverComponent, IGameplayTagAssetInterface), false ));
				OuterClass->StaticLink();
#if WITH_METADATA
				UMetaData* MetaData = OuterClass->GetOutermost()->GetMetaData();
				MetaData->SetValue(OuterClass, TEXT("BlueprintSpawnableComponent"), TEXT(""));
				MetaData->SetValue(OuterClass, TEXT("BlueprintType"), TEXT("true"));
				MetaData->SetValue(OuterClass, TEXT("ClassGroupNames"), TEXT("VRExpansionPlugin"));
				MetaData->SetValue(OuterClass, TEXT("HideCategories"), TEXT("Object Activation Components|Activation Trigger"));
				MetaData->SetValue(OuterClass, TEXT("IncludePath"), TEXT("Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(OuterClass, TEXT("IsBlueprintBase"), TEXT("true"));
				MetaData->SetValue(OuterClass, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_HoldingController, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_HoldingController, TEXT("EditInline"), TEXT("true"));
				MetaData->SetValue(NewProp_HoldingController, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_HoldingController, TEXT("ToolTip"), TEXT("Set on grip notify, not net serializing"));
				MetaData->SetValue(NewProp_bIsHeld, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_bIsHeld, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_bDenyGripping, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_bDenyGripping, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_Damping, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_Damping, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_Stiffness, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_Stiffness, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_BreakDistance, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_BreakDistance, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_MovementReplicationSetting, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_MovementReplicationSetting, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_bReplicateMovement, TEXT("Category"), TEXT("VRGripInterface|Replication"));
				MetaData->SetValue(NewProp_bReplicateMovement, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_bReplicateMovement, TEXT("ToolTip"), TEXT("Overrides the default of : true and allows for controlling it like in an actor, should be default of off normally with grippable components"));
				MetaData->SetValue(NewProp_bRepGameplayTags, TEXT("Category"), TEXT("VRGripInterface"));
				MetaData->SetValue(NewProp_bRepGameplayTags, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_bRepGameplayTags, TEXT("ToolTip"), TEXT("Requires bReplicates to be true for the component"));
				MetaData->SetValue(NewProp_GameplayTags, TEXT("Category"), TEXT("GameplayTags"));
				MetaData->SetValue(NewProp_GameplayTags, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_GameplayTags, TEXT("ToolTip"), TEXT("Tags that are set on this object"));
				MetaData->SetValue(NewProp_bIsLerping, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_bIsLerping, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverReturnSpeed, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_LeverReturnSpeed, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_bSendLeverEventsDuringLerp, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_bSendLeverEventsDuringLerp, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverReturnTypeWhenReleased, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_LeverReturnTypeWhenReleased, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("ClampMax"), TEXT("179.8"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("ClampMin"), TEXT("0.0"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("ToolTip"), TEXT("The max angle of the lever in the negative direction"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("UIMax"), TEXT("180.0"));
				MetaData->SetValue(NewProp_LeverLimitNegative, TEXT("UIMin"), TEXT("0.0"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("ClampMax"), TEXT("179.8"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("ClampMin"), TEXT("0.0"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("ToolTip"), TEXT("The max angle of the lever in the positive direction"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("UIMax"), TEXT("180.0"));
				MetaData->SetValue(NewProp_LeverLimitPositive, TEXT("UIMin"), TEXT("0.0"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("ClampMax"), TEXT("1.0"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("ClampMin"), TEXT("0.01"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("ToolTip"), TEXT("The percentage of the angle at witch the lever will toggle"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("UIMax"), TEXT("1.0"));
				MetaData->SetValue(NewProp_LeverTogglePercentage, TEXT("UIMin"), TEXT("0.01"));
				MetaData->SetValue(NewProp_LeverRotationAxis, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_LeverRotationAxis, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_LeverRotationAxis, TEXT("ToolTip"), TEXT("Rotation axis to use, XY is combined X and Y, only LerpToZero and PositiveLimits work with this mode"));
				MetaData->SetValue(NewProp_bUngripAtTargetRotation, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_bUngripAtTargetRotation, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_bIsPhysicsLever, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_bIsPhysicsLever, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_CurrentLeverForwardVector, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_CurrentLeverForwardVector, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_CurrentLeverForwardVector, TEXT("ToolTip"), TEXT("Bearing Direction, for X/Y is their signed direction, for XY mode it is an actual 2D directional vector"));
				MetaData->SetValue(NewProp_CurrentLeverAngle, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_CurrentLeverAngle, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_CurrentLeverAngle, TEXT("ToolTip"), TEXT("Primary axis angle only"));
				MetaData->SetValue(NewProp_OnLeverStateChanged, TEXT("Category"), TEXT("VRLeverComponent"));
				MetaData->SetValue(NewProp_OnLeverStateChanged, TEXT("ModuleRelativePath"), TEXT("Public/Interactibles/VRLeverComponent.h"));
				MetaData->SetValue(NewProp_OnLeverStateChanged, TEXT("ToolTip"), TEXT("Call to use an object"));
#endif
			}
		}
		check(OuterClass->GetClass());
		return OuterClass;
	}
	IMPLEMENT_CLASS(UVRLeverComponent, 12329742);
	static FCompiledInDefer Z_CompiledInDefer_UClass_UVRLeverComponent(Z_Construct_UClass_UVRLeverComponent, &UVRLeverComponent::StaticClass, TEXT("/Script/VRExpansionPlugin"), TEXT("UVRLeverComponent"), false, nullptr, nullptr, nullptr);
	DEFINE_VTABLE_PTR_HELPER_CTOR(UVRLeverComponent);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#ifdef _MSC_VER
#pragma warning (pop)
#endif
PRAGMA_ENABLE_OPTIMIZATION
